...
void Application::Initialize() {
    ...
    // 注意：set_chat_message 是覆盖式写入，所以必须放在最后
    {
        ...
        switch (esp_reset_reason()) {
            ...
            case ESP_RST_WDT:       crash_msg = "⚠️ 上次崩溃：硬件看门狗"; break;
            case ESP_RST_BROWNOUT:  crash_msg = "⚠️ 上次崩溃：电压不足"; break;
            case ESP_RST_SW:        /* 正常软件重启，不显示 */ break;
            case ESP_RST_POWERON:   /* 正常上电，不显示 */ break;
            default:                crash_msg = "⚠️ 上次异常重启"; break;
        }
        if (crash_msg) {
            display->SetChatMessage("system", crash_msg);
            ESP_LOGW(TAG, "%s", crash_msg);
            vTaskDelay(pdMS_TO_TICKS(5000));  // 等 5 秒让用户看清
...
void Application::ActivationTask() {
    ...
    // Perform OTA version check to get server config (MQTT/WebSocket endpoint, etc.)
    // This contacts the OTA URL configured in Kconfig (CONFIG_OTA_URL),
    // which returns the protocol configuration and stores it in NVS.
    // Without this step, MQTT/WebSocket endpoints are empty after NVS erase.
    CheckNewVersion();

    // Initialize the protocol (uses config from OTA check, or falls back to MQTT default)
    InitializeProtocol();
...
void Application::ContinueOpenAudioChannel(ListeningMode mode) {
    ...
    if (!protocol_) {
        ESP_LOGW(TAG, "Protocol unavailable (null), cannot open audio channel");
        return;
    }
...
void Application::WakeWordInvoke(const std::string& wake_word) {
    ESP_LOGI(TAG, "WakeWordInvoke: state=%d, protocol=%p", (int)GetDeviceState(), (void*)protocol_.get());
    if (!protocol_) {
        return;
    }
    ...
    if (state == kDeviceStateIdle) {
        ...
        // 对话中：注入文本到当前对话，不打断 channel
        // 服务器收到 detect 消息后 LLM 可回应（如"好舒服啊"）
        Schedule([this, wake_word]() {
            if (protocol_) {
                protocol_->SendWakeWordDetected(wake_word);
            }
