...
public:
    PercentageAdjustView(int initialValue, int minValue, int maxValue, int step)
    {
        ...
        _ok_button->label().setText("确定");
        _ok_button->label().setTextFont(GetTextFont());
...
private:
    ...
    {
        int clamped = std::clamp(value, _min_value, _max_value);
        int snapped = _min_value + ((clamped - _min_value + _step / 2) / _step) * _step;
        return std::clamp(snapped, _min_value, _max_value);
    }
...
public:
    explicit ButtonConfigView(const Hal::ButtonConfig& initialConfig) : _current_config(initialConfig)
    {
        ...
        createSwitchRow(195, "按键震动", _current_config.vibrateEnabled,
