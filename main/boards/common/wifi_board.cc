...
WifiBoard::WifiBoard() {
    ...
    // Create 8-second delay timer for wifi config mode (delays entering config on tap)
    esp_timer_create_args_t delay_args = {
        .callback = OnWifiConfigReadyTimer,
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK,
        .name = "wifi_cfg_delay_timer",
        .skip_unhandled_events = true
    };
...
void WifiBoard::TryWifiConnect() {
    ...
    if (have_ssid) {
        ...
        // No SSID configured — do NOT enter config mode immediately.
        // Start an 8-second delay timer. A tap on screen during this window
        // (which calls EnterWifiConfigMode) will enter config mode if wifi_config_ready_ is true.
        wifi_config_ready_ = false;
        ESP_LOGI(TAG, "No saved SSID, starting 8-second delay before allowing config mode");
        esp_timer_start_once(wifi_config_delay_timer_, 8 * 1000000ULL);
    }
}

void WifiBoard::OnWifiConfigReadyTimer(void* arg) {
    auto* board = static_cast<WifiBoard*>(arg);
    board->wifi_config_ready_ = true;
...
void WifiBoard::EnterWifiConfigMode() {
    ...
    // If in Starting state with no saved SSID, check if the 8-second delay has elapsed
    if (state == kDeviceStateStarting) {
        if (!wifi_config_ready_) {
            ESP_LOGI(TAG, "EnterWifiConfigMode ignored — 8-second wifi config delay not yet elapsed");
            return;
        }
        // 8 seconds have passed, proceed to enter config mode
        esp_timer_stop(wifi_config_delay_timer_);
        esp_timer_stop(connect_timer_);
        WifiManager::GetInstance().StopStation();
        StartWifiConfigMode();
