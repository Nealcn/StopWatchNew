...
void LcdDisplay::InitializeLcdThemes() {
    ...
    light_theme->set_assistant_bubble_color(lv_color_hex(0x000000));
...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_style_bg_opa(top_bar_, LV_OPA_TRANSP, 0);  // 透明背景
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    if (strcmp(role, "user") == 0) {
        ...
        // Assistant messages - no background bubble, just transparent
        lv_obj_set_style_bg_opa(msg_bubble, LV_OPA_TRANSP, 0);
        lv_obj_set_style_text_color(msg_text, lvgl_theme->text_color(), 0);
...
void LcdDisplay::SetupUI() {
    ...
    /* chat_message_label_ - single line scrolling for non-multiline mode */
    chat_message_label_ = lv_label_create(bottom_bar_);
    lv_label_set_text(chat_message_label_, "");
    lv_obj_set_width(chat_message_label_, LV_HOR_RES - lvgl_theme->spacing(8));
